...
const Given = ({ formData, handleTextChange, handleAccoTranID, handleTransactionClick, handleAmountTagChange, handleButtonDisable, readOnly }) => {
        ...
        if (!readOnly) {
            ...
            if (formData.amount.slice(-1) === 'R') {
                ...
                for(let i = 0; i < listIDs.length; i++) {
                    let value = transactions.find(transaction => transaction.acco_tran_id === listIDs[i]) || {receivable: '0R'};
                    if(value.receivable) {
                        sum = sum + Number(value.receivable.slice(0, -1));
                    }
                }
        ...
        <>
            ...
            <div className="col border border-3 rounded" style={{position: 'absolute', width: '45%', height: '81px', top: readOnly ? '70%' : '72%', left: '52.5%', minWidth: '186px', maxWidth: '372px', backgroundColor: 'silver'}}>
                <div className="row mx-2 my-2">
                    ...
                    <div className="col">
                        <input type="text" className="form-control form-control-sm" placeholder="0" value={formData.cReceived} name="cReceived" onInput={(e) => handleTextChange(e)} />
                    </div>
                </div>
